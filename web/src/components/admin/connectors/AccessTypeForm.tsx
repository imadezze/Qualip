...
export function AccessTypeForm({
  ...
  const options = [
    ...
    },
    {
      ...
      value: "public",
      description:
        "Everyone with an account on CertiBot can access the documents pulled in by this connector",
      disabled: false,
      disabledReason: "",
    },
  ];
  ...
  if (isAutoSyncSupported && isPaidEnterpriseEnabled) {
    options.push({
      ...
      value: "sync",
      description:
        "We will automatically sync permissions from the source. A document will be searchable in CertiBot if and only if the user performing the search has permission to access the document in the source.",
